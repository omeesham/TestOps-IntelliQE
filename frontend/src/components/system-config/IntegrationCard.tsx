...
const badge: Record<string, { label: string; color: string; bg: string }> = {
  ...
  available: { label: 'Available', color: 'text-[#2143A8]', bg: 'bg-[#EEF4FF] border-[#C5D6FF]' },
...
export default function IntegrationCard({ id, name, category, description, comingSoon, status, connectedBy, lastSyncAt, onConnect, onDisconnect }: Props) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-[#C5D6FF]/60 p-5 hover:shadow-lg hover:shadow-purple-500/5 transition-all">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center border border-gray-100">
            ...
            <span className="text-lg font-bold text-[#2143A8]" style={{ display: logo ? 'none' : '' }}>{name.charAt(0)}</span>
          </div>
          <div>
        ...
        <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${b.bg}`}>
          {status === 'connected' ? <CheckCircle className="w-3 h-3 text-emerald-500" /> : status === 'available' ? <Plug className="w-3 h-3 text-[#2143A8]" /> : <XCircle className="w-3 h-3 text-gray-400" />}
      ...
      <div className="flex items-center justify-between">
        {lastSyncAt && (
          <p className="text-xs text-[#6B7280] flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Synced {timeSince(lastSyncAt)}</p>
        )}
        {connectedBy && <p className="text-xs text-[#6B7280]">by {connectedBy}</p>}
        {status === 'connected' ? (
          ...
          <button onClick={onConnect} className="px-3 py-1.5 bg-[#EEF4FF] text-[#2143A8] rounded-lg text-xs font-medium hover:bg-[#DCE7FF] transition-colors ml-auto">
            Connect
          </button>
        ) : null}
      </div>
