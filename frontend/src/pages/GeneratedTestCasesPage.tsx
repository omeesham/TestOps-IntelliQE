...
import {
  ...
  downloadArtifactZip, listTestCaseFacets,
} from '@/services/api';
...
export default function GeneratedTestCasesPage() {
  ...
  const handleUpdateTc = async () => {
    ...
    try {
      ...
      toast.success('Saved successfully');
    } catch (err) { console.error('Update failed:', err); toast.fromError(err); }
  };
  ...
  const handleAddTc = async () => {
    ...
    try {
      ...
      toast.success('Saved successfully');
    } catch (err) { console.error('Add failed:', err); toast.fromError(err); }
  };
  ...
  const handleDeleteTc = async (caseId: string) => {
    ...
    } catch (err) { console.error('Delete failed:', err); toast.fromError(err); }
  };
  ...
  const handleDeleteRun = async (runId: string) => {
    try {
      ...
      toast.success('Deleted successfully');
    } catch (err) { console.error('Delete run failed:', err); toast.fromError(err); }
  };
  ...
  const handleExport = async (runId: string, format: string) => {
    ...
    } catch (err) { console.error('Export failed:', err); toast.fromError(err); }
  };
                    ...
                    <tr key={tc.id} className={`border-b border-gray-50 hover:bg-gray-50/50 ${isReviewed ? 'bg-emerald-50/30' : ''}`}>
                      ...
                      <td className="px-3 py-1.5 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <ActionIcon tone="view" title="View details" onClick={() => setExpandedTcId(expandedTcId === tc.id ? null : tc.id)}><Eye className="w-4 h-4" /></ActionIcon>
                          <ActionIcon tone="edit" title="Edit" onClick={() => { setEditingTc(tc); setEditDraft({ title: tc.title, steps: tc.steps || [''], expected: tc.expected, priority: tc.priority, type: tc.type, feature: tc.feature, precondition: tc.precondition, status: tc.status }); setExpandedTcId(tc.id); }}><Edit3 className="w-4 h-4" /></ActionIcon>
                          <ActionIcon tone="delete" title="Delete" onClick={() => { setConfirmDelete(tc.id); setConfirmDeleteType('case'); }}><Trash2 className="w-4 h-4" /></ActionIcon>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                return (
                  <tr key={run.id} className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer" onClick={() => openRunDetail(run)}>
                    ...
                    <td className="px-3 py-1.5 text-center" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-1.5">
                        <ActionIcon tone="excel" title="Export CSV" onClick={() => handleExport(run.id, 'csv')}><Download className="w-4 h-4" /></ActionIcon>
                        <ActionIcon tone="delete" title="Delete" onClick={() => { setConfirmDelete(run.id); setConfirmDeleteType('run'); }}><Trash2 className="w-4 h-4" /></ActionIcon>
                      </div>
                    </td>
                  </tr>
                );
