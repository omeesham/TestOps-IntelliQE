            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                  ...
                  <span className="px-1.5 py-0.5 rounded bg-purple-50 text-purple-600 text-[10px]">Automation Script</span>
              ...
              {!isEditing ? (
                <>
                  <button onClick={() => { setIsEditing(true); setEditCode(selectedScript.code); }}
                    className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium bg-[#7C3AED] text-white rounded-lg hover:bg-[#6D28D9]">
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full pl-10 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-300 outline-none bg-white" />
        </div>
        <div className="flex items-center gap-2">
          {runIdFilter && (
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-purple-100 flex items-center justify-center"><Code2 className="w-5 h-5 text-[#7C3AED]" /></div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-12 text-center">
          ...
          <button onClick={() => navigate('/generated-tests')} className="px-4 py-2 text-sm bg-[#7C3AED] text-white rounded-lg hover:bg-[#6D28D9]">
            Go to Test Cases
          </button>
        </div>
      ) : (
                ...
                <tr key={script.id} className="border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer" onClick={() => openScript(script)}>
                  ...
                  <td className="px-4 py-3 text-center" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-center gap-1.5">
                      <ActionIcon tone="view" title="View / Edit" onClick={() => openScript(script)}><Eye className="w-4 h-4" /></ActionIcon>
                      <ActionIcon tone="download" title="Download script" onClick={() => handleDownload(script)}><Download className="w-4 h-4" /></ActionIcon>
                      <ActionIcon tone="delete" title="Delete" onClick={() => setConfirmDelete(script.id)}><Trash2 className="w-4 h-4" /></ActionIcon>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          {!runIdFilter && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 text-xs text-gray-500">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="px-2 py-1 bg-[#7C3AED] text-white rounded text-[10px] font-bold">{pagination.page}</span>
