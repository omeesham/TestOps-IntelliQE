...
- Name: tc-005-password-masked-not-exposed.spec.ts >> TC-005 | Password Input Masking >> password is masked and not exposed in DOM, source, or accessibility tree
- Location: ..\..\..\..\Users\VAMSEE~1\AppData\Local\Temp\jbs-pwexec-9ECA8BE5-7668-4F22-A56E-22B158F7C8C0-1782550083607\tests\tc-005-password-masked-not-exposed.spec.ts:64:7
...
Expected substring: not "admin123"
Received string:        "<!DOCTYPE html><html><head>
  <meta charset=\"UTF-8\">
  <meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\">
  <meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">
  <title>OrangeHRM</title>
      <link rel=\"icon\" href=\"/web/dist/favicon.ico?v=1763650546848\">
    <link href=\"/web/dist/css/chunk-vendors.css?v=1763650546848\" rel=\"preload\" as=\"style\">
    <link href=\"/web/dist/css/app.css?v=1763650546848\" rel=\"preload\" as=\"style\">
    <link href=\"/web/dist/js/chunk-vendors.js?v=1763650546848\" rel=\"preload\" as=\"script\">
    <link href=\"/web/dist/js/app.js?v=1763650546848\" rel=\"preload\" as=\"script\">
      <link href=\"/web/dist/css/chunk-vendors.css?v=1763650546848\" rel=\"stylesheet\">
    <link href=\"/web/dist/css/app.css?v=1763650546848\" rel=\"stylesheet\">
</head>
<body>
  <style>
    :root {
    ...
    <script src=\"/web/dist/js/chunk-vendors.js?v=1763650546848\"></script>
    <script src=\"/web/dist/js/app.js?v=1763650546848\"></script>
...
```ts
  ...
  4  | 
  5  | /**
  6  |  * Page Object for the OrangeHRM login screen, focused on password-field security.
  7  |  */
  8  | class LoginPage {
  ...
  15 |     this.page = page;
  16 |     // OrangeHRM inputs expose a stable placeholder; a password input has no 'textbox' role,
  17 |     // so getByPlaceholder is the most reliable accessibility-first locator here.
  ...
  25 |   }
  26 | 
  27 |   async expectLoginFormVisible(): Promise<void> {
  ...
  31 |   }
  32 | 
  33 |   async typePassword(password: string): Promise<void> {
  34 |     await this.passwordInput.fill(password);
  35 |   }
  36 | 
  37 |   /** Step 2/3: the field renders masked because its type is 'password'. */
  38 |   async expectPasswordMasked(): Promise<void> {
  39 |     await expect(this.passwordInput).toHaveAttribute('type', 'password');
  40 |   }
  41 | 
  42 |   /** Functional proof the value was actually entered (held only in the .value property). */
  43 |   async expectValueEntered(expected: string): Promise<void> {
  44 |     await expect(this.passwordInput).toHaveValue(expected);
  45 |   }
  46 | 
  47 |   /** Step 3: the typed secret must not leak into the serialized HTML or the value attribute. */
  48 |   async expectPlaintextNotInDom(secret: string): Promise<void> {
  49 |     const html = await this.page.content();
> 50 |     expect(html).not.toContain(secret);
     |                      ^ Error: expect(received).not.toContain(expected) // indexOf
  51 |     // The HTML 'value' attribute (distinct from the live .value property) must not carry the secret.
  52 |     const valueAttr = await this.passwordInput.getAttribute('value');
  53 |     expect(valueAttr ?? '').not.toContain(secret);
  54 |   }
  55 | 
  56 |   /** Step 4: the accessibility tree exposes a secure entry, never the literal characters. */
  57 |   async expectSecureAccessibility(secret: string): Promise<void> {
  58 |     const snapshot = await this.passwordInput.ariaSnapshot();
  59 |     expect(snapshot).not.toContain(secret);
  60 |   }
  61 | }
  62 | 
  63 | test.describe('TC-005 | Password Input Masking', () => {
  64 |   test('password is masked and not exposed in DOM, source, or accessibility tree', async ({ page }) => {
  65 |     const password = 'admin123';
  66 |     const loginPage = new LoginPage(page);
  67 | 
  68 |     // Step 1: Navigate to the login page -> the login form is displayed.
  69 |     await loginPage.goto();
  70 |     await loginPage.expectLoginFormVisible();
  71 | 
  72 |     // Step 2: Type the password -> rendered masked (type=password) while the value is captured.
  73 |     await loginPage.typePassword(password);
  74 |     await loginPage.expectPasswordMasked();
  75 |     await loginPage.expectValueEntered(password);
  76 | 
  77 |     // Step 3: Inspect the DOM -> type=password and no plaintext value attribute / source leak.
  78 |     await loginPage.expectPlaintextNotInDom(password);
  79 | 
  80 |     // Step 4: Inspect the accessibility tree -> secure entry, literal value not announced.
  81 |     await loginPage.expectSecureAccessibility(password);
  82 |   });
  83 | });
  84 | 
```