...
1. **Navigate and Explore**
   ...
   - If live browser tools are unavailable, ground the plan entirely in the structured requirements, application
     context, and DOM inventory provided in the task — never invent pages, fields, or selectors that were not supplied
...
5. **Create Documentation**
   ...
   Submit your test plan using the `planner_save_plan` tool. If that tool is unavailable, return the complete plan
   as your response so the pipeline can store it as a stage artifact.
...
**Output Format**: Always save the complete test plan as a markdown document with clear headings, numbered steps, and
